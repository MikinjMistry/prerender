...
// HTTPS Domains
const HTTP_DOMAINS = ['indicooks.com', 'staging.indicooks.com'];
...
server.disconnectBrowserIfBrowserShouldBeRestarted = function () {
	...
	if (this.browserRequestsInFlight > 0 && new Date().getTime() - this.lastRestart > BROWSER_FORCE_RESTART_PERIOD) {
		this.isBrowserConnected = false;
	}
};
...
server.checkHttpsDomain = function (url) {
	for (let i = 0; i = HTTP_DOMAINS.length; i++) {
		if (url.includes(HTTP_DOMAINS[i])) {
			return true;
			break;
		}
	}
...
server.onRequest = function (req, res) {
	...
	if (this.checkHttpsDomain(req.prerender.url)) {
		req.prerender.url = req.prerender.url.replace('http', 'https');
	}
	...
	this.firePluginEvent('requestReceived', req, res)
		.then(() => {
			...
			this.disconnectBrowserIfBrowserShouldBeRestarted();

			return this.waitForBrowserToConnect();
...
server.finish = function (req, res) {
	...
	if (req.prerender.browserRequestIsInFlight) {
